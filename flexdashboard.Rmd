...
renderPlotly({
   if (input$rank_pos) 
    if (input$d == "All Years")
    ps |>
  plot_ly(x = ~Poss, y = ~.data[[input$y2]], trendline = "ols", type = 'scatter', mode = 'markers', color = ~Rank, text = ~paste("Team: ", Squad, '<br>Year: ', Year)) |>
    add_lines(x = ~Poss, y = fitted(lm))
    else 
       ps |>
    filter(Year %in% c(input$d)) |>
   plot_ly(x = ~Poss, y = ~.data[[input$y2]], type = 'scatter', mode = 'markers')
  else if (!input$rank_pos)
    if (input$d == "All Years")
    ps |>
       plot_ly(x = ~Poss, y = ~.data[[input$y2]], type = 'scatter', mode = 'markers')
